...
class SimulatorBridge:
    ...
    def __init__(self):
        ...
        # Read parameters
        self.wheels_radius = rospy.get_param("/vehicle/wheel_radius",
                                             default=0.03)
        self.wheels_perimeter = self.wheels_radius * 2 * pi

        # Init Subscribers
        self.sub_manual_drive = rospy.Subscriber("/drive/manual", DriveCommand,
                                                 self.manual_drive_callback)
        self.sub_drive = rospy.Subscriber("/selfie_in/drive", DriveCommand,
                                          self.drive_callback)
        self.sub_joints_states = rospy.Subscriber("/vehicle/joint_states",
                                                  JointState,
                                                  self.send_speed_and_distance)
        self.sub_indicators = rospy.Subscriber("/selfie_in/indicators",
                                               Indicators,
                                               self.indicators_callback)
        self.sub_back_dist = rospy.Subscriber("/vehicle/back_distance",
                                              LaserScan,
                                              self.back_dist_callback)

        # Init Subscribers
        self.sub_drive_mode = rospy.Subscriber("/simulation/switch_state",
                                               Int8, self.change_mode_callback)
        self.sub_imu = rospy.Subscriber('/imu', Imu, self.imu_callback)

        # Init publishers
        self.pub_movement = rospy.Publisher('/selfie_out/motion',
                                            Motion,
                                            queue_size=1)
        self.pub_drive_mode = rospy.Publisher("/state/rc", Int8, queue_size=1)
        self.pub_back_dist = rospy.Publisher("/back_distance",
                                             Float64,
                                             queue_size=1)

        self.pub_vel_left_rear_wheel = rospy.Publisher(
            '/vehicle/left_rear_wheel_velocity_controller/command',
            Float64,
            queue_size=1)
        self.pub_vel_right_rear_wheel = rospy.Publisher(
            '/vehicle/right_rear_wheel_velocity_controller/command',
            Float64,
            queue_size=1)
        self.pub_vel_left_front_wheel = rospy.Publisher(
            '/vehicle/left_front_wheel_velocity_controller/command',
            Float64,
            queue_size=1)
        self.pub_vel_right_front_wheel = rospy.Publisher(
            '/vehicle/right_front_wheel_velocity_controller/command',
            Float64,
            queue_size=1)
        self.pub_pos_left_front_steering_hinge = rospy.Publisher(
            '/vehicle/left_front_steering_hinge_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_pos_right_front_steering_hinge = rospy.Publisher(
            '/vehicle/right_front_steering_hinge_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_pos_left_rear_steering_hinge = rospy.Publisher(
            '/vehicle/left_rear_steering_hinge_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_pos_right_rear_steering_hinge = rospy.Publisher(
            '/vehicle/right_rear_steering_hinge_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_right_indicator = rospy.Publisher('/sim_right_turn_indicator',
                                                   Bool,
                                                   queue_size=1)
        self.pub_left_indicator = rospy.Publisher('/sim_left_turn_indicator',
                                                  Bool,
                                                  queue_size=1)
    ...
    def change_mode_callback(self, msg: UInt8):
        try:
            ...
            rospy.logerr("Wrong value of drive mode message! Current value: " +
                         msg.data)
            return
